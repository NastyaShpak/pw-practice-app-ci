...
test('Locator symtax rules', async({page}) => {
    //by Tag name
...
test('Locating child components', async({page}) => {
    await page.locator('nb-card nb-radio :text-is("Option 1")').click()
    await page.locator('nb-card').locator('nb-radio').locator(':text-is("Option 2")').click()
    await page.locator('nb-card').getByRole('button', {name: "Sign in"}).first().click()
    await page.locator('nb-card').nth(3).getByRole('button').click() // least preferable, because amount of elements can be change
})

test('Locating parent components', async({page}) => {
...
test.skip('Assertions', async({page}) => {
    ...
    //soft assertions (will execute next command)
